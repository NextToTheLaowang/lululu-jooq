...
public class NumberUtil {
    ...
    private static final BigDecimal MONTH_LEN = new BigDecimal(12);
    ...
    /**
     * 平台按日 分成
     * 平台佣金=（（本金*佣金比例）/365）*（提前还款日期-上期还款日）
     *
     * @return
     */
    public static BigDecimal platformLoanPointInterest(BigDecimal amount, BigDecimal annualizedRate, int
            date) {
        BigDecimal divide = annualizedRate.divide(YEAR_DAYS, 10, BigDecimal.ROUND_HALF_UP);
        BigDecimal pay = amount.multiply(divide).multiply(new BigDecimal(date)).setScale(2, BigDecimal.ROUND_HALF_UP);
        return pay;
     ...
     * 格式化金额，中间无分隔符“,”
     */
    public static String formatPriceNoSep(BigDecimal price) {
        if (price == null) {
            return "";
        }
    ...
    public static String getIntegerDefaultZero(BigDecimal price) {
        String priceText = getInteger(price);
        if (StringTools.isNotBlank(priceText)) {
            return priceText;
        }
        return "0";
    }
    ...
    public static String getDecimalDefaultZero(BigDecimal price) {
        String priceText = getDecimal(price);
        if (StringTools.isNotBlank(priceText)) {
            return priceText;
        }
        return ".00";
     ...
     * @return
     */
    public static BigDecimal retu(BigDecimal bigDecimal) {
        if (bigDecimal == null) {
            return null;
        }
        BigDecimal multiply = new BigDecimal("100");
        BigDecimal bigDecimalMultiply = bigDecimal.multiply(multiply);
        BigDecimal bd = bigDecimalMultiply
                .setScale(2, BigDecimal.ROUND_HALF_UP);
        return bd;
    }
     ...
     * @return
     */
    public static BigDecimal re(BigDecimal bigDecimal) {
        if (bigDecimal != null) {
            BigDecimal divide = new BigDecimal("100");
            BigDecimal bigDecimalDivide = bigDecimal.divide(divide);
            // 保留四位小数
            BigDecimal bd = bigDecimalDivide.setScale(4,
                    BigDecimal.ROUND_HALF_UP);
            return bd;
        }
    ...
    public static void main(String[] arg) {
        String s = NumberUtil.formatPriceNoSep(getPointScale(new BigDecimal("6700")));
